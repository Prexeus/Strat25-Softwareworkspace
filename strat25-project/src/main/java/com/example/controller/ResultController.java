...
public class ResultController {
    ...
    @FXML private StackedBarChart<Number, String> prestigeChart;
    @FXML private NumberAxis prestigeXAxis;
    @FXML private CategoryAxis prestigeYAxis;

    // ===== Zustand =====
    private BuildCategory activeBuild;
    private boolean showEndscore;

    // Merker für zuletzt gesetzte Sichtbarkeit (für Force-Refresh)
    private Set<String> lastVisibleCategoryNames = Set.of();
    ...
    private void initialize() {
        ...
        hookRefreshOnWindowShown(); // <<< NEU: beim Anzeigen / Ent-Minimieren hart neu aufbauen
    ...
    /** NEU: Wenn das Fenster angezeigt oder ent-minimiert wird → alles hart neu laden. */
    private void hookRefreshOnWindowShown() {
        Node anchor = (imageHolder != null) ? imageHolder : activeBuildLabel;
        if (anchor == null) return;

        anchor.sceneProperty().addListener((obs, oldScene, scene) -> {
            if (scene == null) return;
            scene.windowProperty().addListener((o2, oldWin, win) -> {
                if (win == null) return;
                // Beim Anzeigen
                win.showingProperty().addListener((o3, wasShowing, isShowing) -> {
                    if (isShowing) forceRefreshAll();
                });
                // Beim Ent-Minimieren
                try {
                    // Stage hat iconifiedProperty – über Window-API erreichbar
                    var iconifiedProp = win.getClass().getMethod("iconifiedProperty");
                    var prop = iconifiedProp.invoke(win);
                    if (prop instanceof javafx.beans.property.ReadOnlyBooleanProperty p) {
                        p.addListener((o4, wasIcon, isIcon) -> {
                            if (!isIcon) forceRefreshAll();
                        });
                    }
                } catch (Exception ignored) { /* nicht kritisch */ }
            });
        });
    }
    ...
    public void applyVisibility(Set<String> visibleCategoryNames, boolean showEndscore) {
        ...
        this.lastVisibleCategoryNames = (visibleCategoryNames == null)
                ? Set.of()
                : new LinkedHashSet<>(visibleCategoryNames);
    ...
    /** NEU: Von überall aufrufbar – baut ALLES hart neu (Charts, Bilder, Farben). */
    public void forceRefreshAll() {
        ...
        lastMaterialsLines = List.of();
        lastPieSignature = "";
        lastPrestigeSignature = "";

        // Extra-Caches
        extraPieSignatureByName.clear();
        extraIconUrlByName.clear();

        // Linke Seite anhand zuletzt gemerkter Auswahl neu aufbauen
        List<CategoryInterface> extras = firstFourNonBuildCategories(lastVisibleCategoryNames);
        rebuildExtraChartsGridIfNeeded(extras);

        // Danach ein kompletter Refresh
        safeRefresh();
    }
    ...
    private void rebuildMainInfluencePie(Map<Integer, Double> influence) {
        ...
        double sum = influence.values().stream().filter(Objects::nonNull).mapToDouble(Double::doubleValue).sum();
    ...
    private List<CategoryInterface> firstFourNonBuildCategories(Set<String> visibleNames) {
        ...
        for (CategoryInterface ci : g.getCategories()) {
            if (ci instanceof BuildCategory) continue;
            String nm = ci.getName();
            if (nm == null || !visibleNames.contains(nm)) continue;
            out.add(ci);
            if (out.size() == 4) break;
        }
        return out;
    ...
    private void rebuildExtraChartsGridIfNeeded(List<CategoryInterface> extras) {
        ...
        if (names.equals(lastExtraNames)) return;
    ...
    private void updateExtraChartsData() {
        ...
        for (String catName : extraChartsByName.keySet()) {
            ...
            if (title != null) title.setText(ci != null && ci.getName() != null ? ci.getName() : catName);
    ...
    private void rebuildPrestigeChart() {
        ...
        if (sig.equals(lastPrestigeSignature)) return;
        ...
        for (Family f : families) {
            ...
            for (Team t : f.getTeams()) {
                if (t != null && t.getId() != null) allTeams.put(t.getId(), t);
            }
        }
            ...
            for (StackedBarChart.Data<Number, String> d : s.getData()) {
                if (d.getNode() != null) {
                    ...
                    d.nodeProperty().addListener((obs, oldN, newN) -> {
                        if (newN != null) newN.setStyle("-fx-bar-fill: " + cssColor + ";");
                    });
                }
            }
        }
